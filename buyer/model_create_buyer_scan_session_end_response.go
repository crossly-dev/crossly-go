/*
Crossly Buyer API

The buyer-facing Crossly API — cart, orders, wishlists, offers, cashback. A DIFFERENT principal from the seller API: authenticated by a buyer OAuth token scoped `buyer:*` and resolved by `resolveBuyer`. A seller Personal Access Token will NOT authenticate these endpoints.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package crossly_buyer

import (
	"encoding/json"
)

// CreateBuyerScanSessionEndResponse struct for CreateBuyerScanSessionEndResponse
type CreateBuyerScanSessionEndResponse struct {
	Id string `json:"id"`
	Device string `json:"device"`
	Label NullableString `json:"label,omitempty"`
	StartedAt string `json:"startedAt"`
	EndedAt NullableString `json:"endedAt,omitempty"`
	CaptureCount float32 `json:"captureCount"`
}

// NewCreateBuyerScanSessionEndResponse instantiates a new CreateBuyerScanSessionEndResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateBuyerScanSessionEndResponse(id string, device string, startedAt string, captureCount float32) *CreateBuyerScanSessionEndResponse {
	this := CreateBuyerScanSessionEndResponse{}
	this.Id = id
	this.Device = device
	this.StartedAt = startedAt
	this.CaptureCount = captureCount
	return &this
}

// NewCreateBuyerScanSessionEndResponseWithDefaults instantiates a new CreateBuyerScanSessionEndResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateBuyerScanSessionEndResponseWithDefaults() *CreateBuyerScanSessionEndResponse {
	this := CreateBuyerScanSessionEndResponse{}
	return &this
}

// GetId returns the Id field value
func (o *CreateBuyerScanSessionEndResponse) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *CreateBuyerScanSessionEndResponse) GetIdOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *CreateBuyerScanSessionEndResponse) SetId(v string) {
	o.Id = v
}

// GetDevice returns the Device field value
func (o *CreateBuyerScanSessionEndResponse) GetDevice() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Device
}

// GetDeviceOk returns a tuple with the Device field value
// and a boolean to check if the value has been set.
func (o *CreateBuyerScanSessionEndResponse) GetDeviceOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Device, true
}

// SetDevice sets field value
func (o *CreateBuyerScanSessionEndResponse) SetDevice(v string) {
	o.Device = v
}

// GetLabel returns the Label field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CreateBuyerScanSessionEndResponse) GetLabel() string {
	if o == nil || o.Label.Get() == nil {
		var ret string
		return ret
	}
	return *o.Label.Get()
}

// GetLabelOk returns a tuple with the Label field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CreateBuyerScanSessionEndResponse) GetLabelOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.Label.Get(), o.Label.IsSet()
}

// HasLabel returns a boolean if a field has been set.
func (o *CreateBuyerScanSessionEndResponse) HasLabel() bool {
	if o != nil && o.Label.IsSet() {
		return true
	}

	return false
}

// SetLabel gets a reference to the given NullableString and assigns it to the Label field.
func (o *CreateBuyerScanSessionEndResponse) SetLabel(v string) {
	o.Label.Set(&v)
}
// SetLabelNil sets the value for Label to be an explicit nil
func (o *CreateBuyerScanSessionEndResponse) SetLabelNil() {
	o.Label.Set(nil)
}

// UnsetLabel ensures that no value is present for Label, not even an explicit nil
func (o *CreateBuyerScanSessionEndResponse) UnsetLabel() {
	o.Label.Unset()
}

// GetStartedAt returns the StartedAt field value
func (o *CreateBuyerScanSessionEndResponse) GetStartedAt() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.StartedAt
}

// GetStartedAtOk returns a tuple with the StartedAt field value
// and a boolean to check if the value has been set.
func (o *CreateBuyerScanSessionEndResponse) GetStartedAtOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.StartedAt, true
}

// SetStartedAt sets field value
func (o *CreateBuyerScanSessionEndResponse) SetStartedAt(v string) {
	o.StartedAt = v
}

// GetEndedAt returns the EndedAt field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CreateBuyerScanSessionEndResponse) GetEndedAt() string {
	if o == nil || o.EndedAt.Get() == nil {
		var ret string
		return ret
	}
	return *o.EndedAt.Get()
}

// GetEndedAtOk returns a tuple with the EndedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CreateBuyerScanSessionEndResponse) GetEndedAtOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.EndedAt.Get(), o.EndedAt.IsSet()
}

// HasEndedAt returns a boolean if a field has been set.
func (o *CreateBuyerScanSessionEndResponse) HasEndedAt() bool {
	if o != nil && o.EndedAt.IsSet() {
		return true
	}

	return false
}

// SetEndedAt gets a reference to the given NullableString and assigns it to the EndedAt field.
func (o *CreateBuyerScanSessionEndResponse) SetEndedAt(v string) {
	o.EndedAt.Set(&v)
}
// SetEndedAtNil sets the value for EndedAt to be an explicit nil
func (o *CreateBuyerScanSessionEndResponse) SetEndedAtNil() {
	o.EndedAt.Set(nil)
}

// UnsetEndedAt ensures that no value is present for EndedAt, not even an explicit nil
func (o *CreateBuyerScanSessionEndResponse) UnsetEndedAt() {
	o.EndedAt.Unset()
}

// GetCaptureCount returns the CaptureCount field value
func (o *CreateBuyerScanSessionEndResponse) GetCaptureCount() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.CaptureCount
}

// GetCaptureCountOk returns a tuple with the CaptureCount field value
// and a boolean to check if the value has been set.
func (o *CreateBuyerScanSessionEndResponse) GetCaptureCountOk() (*float32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.CaptureCount, true
}

// SetCaptureCount sets field value
func (o *CreateBuyerScanSessionEndResponse) SetCaptureCount(v float32) {
	o.CaptureCount = v
}

func (o CreateBuyerScanSessionEndResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["id"] = o.Id
	}
	if true {
		toSerialize["device"] = o.Device
	}
	if o.Label.IsSet() {
		toSerialize["label"] = o.Label.Get()
	}
	if true {
		toSerialize["startedAt"] = o.StartedAt
	}
	if o.EndedAt.IsSet() {
		toSerialize["endedAt"] = o.EndedAt.Get()
	}
	if true {
		toSerialize["captureCount"] = o.CaptureCount
	}
	return json.Marshal(toSerialize)
}

type NullableCreateBuyerScanSessionEndResponse struct {
	value *CreateBuyerScanSessionEndResponse
	isSet bool
}

func (v NullableCreateBuyerScanSessionEndResponse) Get() *CreateBuyerScanSessionEndResponse {
	return v.value
}

func (v *NullableCreateBuyerScanSessionEndResponse) Set(val *CreateBuyerScanSessionEndResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateBuyerScanSessionEndResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateBuyerScanSessionEndResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateBuyerScanSessionEndResponse(val *CreateBuyerScanSessionEndResponse) *NullableCreateBuyerScanSessionEndResponse {
	return &NullableCreateBuyerScanSessionEndResponse{value: val, isSet: true}
}

func (v NullableCreateBuyerScanSessionEndResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateBuyerScanSessionEndResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


